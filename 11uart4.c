...
//static void CheckAndRecordAlarm1(void);
...
static unsigned char GetPage2StartIndex(void) {
    ...
    // 选中项≥2时，起始索引=选中项-2（保证选中项在显示的第3个位置）
    if (menu_state.page2_selected >= 2) {
        start_idx = menu_state.page2_selected - 2;
    }
    // 边界保护：起始索引不能超过最大可用值（避免显示空项）
    if (start_idx > (PAGE2_ITEM_COUNT - 3)) {
        start_idx = PAGE2_ITEM_COUNT - 3;
    }
    return start_idx;
}
...
static void UpdatePage2MenuItems(unsigned char start_idx) {
    unsigned char i;
    for (i = 0; i < 3; i++) {
        unsigned char item_idx = start_idx + i; // 当前要显示的菜单项索引
        ...
        switch(item_idx) {
            case MENU_MEASURE_DATA: // 0：测量数据
                ...
                break;
            case MENU_SENSOR_STATUS: // 1：传感器状态
            ...
            case MENU_PARAM_QUERY: // 2：参数查询
                ...
                break;
            case MENU_HISTORY_RECORD: // 3：历史记录
...
// 修改原DisplayPage2函数：仅做分发
static void DisplayPage2(void) {
	static unsigned char last_start_idx;
    unsigned char start_idx = GetPage2StartIndex(); // 实时计算起始索引
    if (display_labels_initialized == 0) {
        LCD_Clear();          // 仅首次初始化时清屏
        // 绘制功能提示（仅首次绘制）
        LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);   // "下"
...
static unsigned char GetPage7StartIndex(void) {
    ...
    // 边界保护：不超过最大可用起始索引
    if (start_idx > (PAGE7_ITEM_COUNT - 3)) {
        start_idx = PAGE7_ITEM_COUNT - 3;
    }
    return start_idx;
}
...
static void UpdatePage7MenuItems(unsigned char start_idx) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        // 根据索引显示对应的菜单项文本（复用原有 PAGE_7 文本逻辑）
...
static void DisplayPage7(void) {
	static unsigned char last_start_idx;
    unsigned char start_idx = GetPage7StartIndex(); // 实时计算起始索引
    if (display_labels_initialized == 0) {
//        LCD_Clear();          // 仅首次初始化时清屏
        // 绘制功能提示（仅首次绘制）
        LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);   // "下"
    ...
    // 仅刷新箭头和必要的文本同步（核心优化：减少冗余）
    RefreshPage7Arrow(); // 优先刷新箭头，实时响应
    // 仅当起始索引变化时更新文本（避免无意义重复绘制）
     last_start_idx = 0;
    if (start_idx != last_start_idx) {
        UpdatePage7MenuItems(start_idx);
        last_start_idx = start_idx;
...
static void DisplayPage22(void) {
    ...
    // 3. 清屏并绘制详情页固定标签（完全复用 PAGE_11 布局）
    LCD_Clear();
    ...
    // 显示选中行的最高温数据
    if (max_temps[selected_row] != -990) {
        // 温度：放大10倍转换为整数（如 250→25℃）
        temp = (unsigned char)(max_temps[selected_row] / 10);
        // PID/AID
        LCD_DisplayNumber(0, 56, pid, 2);
        LCD_DisplayNumber(0, 104, aid, 2);
        // 温度+时间戳（月-日 HH:MM）
...
static void HandlePrevItem(void)
{
    ...
    switch(menu_state.current_page) {
        ...
        menu_state.page2_selected--; // 立即更新选中索引
         start_idx = GetPage2StartIndex();
        // 仅起始索引变化时更新文本
        last_prev_start_idx = 0;
        if (start_idx != last_prev_start_idx) {
            UpdatePage2MenuItems(start_idx);
            last_prev_start_idx = start_idx;
        }
        RefreshPage2Arrow(); // 按键后立即刷箭头，无延迟
        menu_state.page_changed = 1;
    }
    break;
        ...
        menu_state.page7_selected--; // 立即更新选中索引
        start_idx = GetPage7StartIndex();
        // 仅起始索引变化时更新文本（避免冗余）
        last_prev_start_idx = 0;
        if (start_idx != last_prev_start_idx) {
            UpdatePage7MenuItems(start_idx);
        ...
        RefreshPage7Arrow(); // 按键后立即刷新箭头，无延迟
        menu_state.page_changed = 1;
    }
    break;
				case PAGE_8:
...
static void HandleNextItem(void)
{
    ...
    switch(menu_state.current_page) {
        ...
        start_idx = GetPage2StartIndex();
        // 仅起始索引变化时更新文本
       last_next_start_idx = 0;
        if (start_idx != last_next_start_idx) {
            UpdatePage2MenuItems(start_idx);
            last_next_start_idx = start_idx;
        }
        RefreshPage2Arrow(); // 按键后立即刷箭头，无延迟
        menu_state.page_changed = 1;
    }
