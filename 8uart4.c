...
// ------------------- 显示PAGE_21(最高温度查询页面) -------------------
static void DisplayPage21(void) {
    // 1. 适配现有LCD驱动：3行数据对应LCD的页0、页2、页4（每行列间隔1页，避免8x16字符跨页重叠）
    const unsigned char row_page[3] = {0, 2, 4}; 
    // 2. 独立闪烁控制：仅选中行闪烁，未选中行强制显示
    static unsigned int flash_timer = 0;
    static bit flash_state = 1;          // 初始=1：默认显示数据（解决开机清空问题）
    const unsigned int FLASH_INTERVAL = 25; // 闪烁周期（25次调用=约250ms，可调整）
    short max_temps[3];
		int i;
    ...
    LCD_Clear(); // 清屏（使用原有驱动的清屏函数，确保无残留）
    
    // 4. 逐行处理（适配现有LCD_DisplayString/LCD_DisplayNumber函数）
    for (i = 0; i < 3; i++) {
        unsigned char curr_page = row_page[i]; // 当前行对应的LCD页
        int selected_row = menu_state.page21_selected; // 0/1/2
        
        if (i == selected_row) { // 选中行：闪烁控制
            if (flash_state) { // 显示数据（大部分时间）
                if (max_temps[i] != -990) {
                    // 温度：使用原有LCD_DisplayNumber（8x16字符，跨页显示）
                    LCD_DisplayNumber(curr_page, 0, (unsigned int)(max_temps[i]/10), 2);
//                    LCD_DisplayChar(curr_page, 16, 'C'); // 温度单位
                    LCD_DISPLAYCHAR_NEW(curr_page, 16, 0, 2);  // "摄氏度"
                    // 日期：月-日（适配列地址，避免字符重叠）
                    LCD_DisplayNumber(curr_page, 32, (unsigned int)max_temp_times[i].mon, 2);
//                    LCD_DisplayChar(curr_page, 48, '-');
									LCD_DISPLAYCHAR_NEW(curr_page, 48, 1, 15); // "."                  // "."  
                    LCD_DisplayNumber(curr_page, 56, (unsigned int)max_temp_times[i].day, 2);
                    
                    // 时间：时:分
                    LCD_DisplayNumber(curr_page, 88, (unsigned int)max_temp_times[i].hour, 2);
//                    LCD_DisplayChar(curr_page, 88, ':');
									LCD_DISPLAYCHAR_NEW(curr_page, 104, 0, 15); // ":"
                    LCD_DisplayNumber(curr_page, 112, (unsigned int)max_temp_times[i].min, 2);
                } else {
                    // 无数据：显示占位符（使用原有LCD_DisplayString）
                    LCD_DisplayString(curr_page, 0, (unsigned char*)"  C -- -- --:--");
                }
            } else { // 闪烁清空（仅5帧，使用原有清页函数）
                LCD_ClearPages(curr_page, curr_page); // 清空当前行对应的页
                // 清空后显示空占位符（避免全黑，保持视觉一致性）
    ...
    // 5. 功能提示（复用原有LCD_DISPLAYCHAR_NEW函数，保持风格一致）
    LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);    // "下"
    LCD_DISPLAYCHAR_NEW(6, 8, 1, 4);    // "一"
    LCD_DISPLAYCHAR_NEW(6, 16, 2, 4);   // "页"
    LCD_DISPLAYCHAR_NEW(6, 40, 3, 4);   // "上"
