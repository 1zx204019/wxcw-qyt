...
static void DisplayPage2(void) {
    ...
    // 每次调用都重新更新菜单项文本和箭头（关键：强制同步）
    UpdatePage2MenuItems(start_idx); // 同步显示文本
    RefreshPage2Arrow();            // 同步箭头位置
}
...
static void DisplayPage7(void) {
    ...
    // 每次调用都重新更新菜单项文本和箭头（强制同步，和 PAGE_2 一致）
    UpdatePage7MenuItems(start_idx); // 同步显示文本
    RefreshPage7Arrow();            // 同步箭头位置
}
    ...
    switch(menu_state.current_page) {
        case PAGE_2:
          if (menu_state.page2_selected > 0) {
                menu_state.page2_selected--; // 先更新实际选中项索引
                // 强制同步显示：更新文本+箭头（局部刷新，不清屏）
        ...
        menu_state.page7_selected--; // 更新选中索引
        // 局部刷新：仅更新文本和箭头，不清屏（和 PAGE_2 一致）
...
static void RefreshDisplay(void)
{
    if (menu_state.page_changed) {
        ...
        if (menu_state.current_page != PAGE_2 && menu_state.current_page != PAGE_7) {
            ...
            // PAGE_7 同 PAGE_2 逻辑：仅局部刷新箭头，不清屏
            RefreshPage7Arrow();
        }
