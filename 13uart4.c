...
static void DisplayPage22(void) {
    ...
    if (max_temps[selected_row] != -990 && daily_max_temps[selected_row].is_valid) {
        // 有有效数据：正常显示（原有逻辑保留）
        ...
        LCD_DISPLAYCHAR_NEW(2, 40, 0, 2); // "℃"符号
        LCD_DisplayNumber(2, 88, max_temp_times[selected_row].mon, 2);
        LCD_DISPLAYCHAR_NEW(2, 104, 1, 15); // "."分隔符
        LCD_DisplayNumber(2, 112, max_temp_times[selected_row].day, 2);
        // 电压（mV）
        LCD_DisplayNumber(4, 24, volt_mv, 4);
    } else {
        // 数据已清空：显示占位符“--”，不改动固定字符
        LCD_DisplayString(0, 56, (unsigned char*)"--");  // PID占位
        LCD_DisplayString(0, 112, (unsigned char*)"--"); // AID占位
        LCD_DisplayString(2, 24, (unsigned char*)"--");  // 温度占位
        LCD_DisplayString(2, 88, (unsigned char*)"-- --"); // 时间占位（月-日格式）
        LCD_DisplayString(4, 24, (unsigned char*)"----"); // 电压占位
    }
}
...
////                ClearMaxTemperatureData();
        ...
        DeleteMaxTempEvent(menu_state.page21_selected);
    }
    
    // 如果要清空所有数据（可选）
    // ClearAllMaxTempEvents();
...
static void GetMaxTempRecords_Ext(short max_temps[], rtc_time_t max_temp_times[], 
     ...
     unsigned char selected_row;
		 unsigned char i;
																	    // 先更新每日最高温数据
    CheckDailyMaxTemp();
    selected_row = menu_state.page21_selected; // 关联PAGE_21选中行
    *pid = 0;
    *aid = 0;
    *volt_mv = 0;
    *target_record = NULL;
    ...
    // 为选中行赋值对应的PID/AID/电压
    if (daily_max_temps[selected_row].is_valid) {
        *pid = daily_max_temps[selected_row].pid;
        *aid = daily_max_temps[selected_row].aid;
        *volt_mv = daily_max_temps[selected_row].volt_mv;
    }
}
    ...
    for (slave_idx = 0; slave_idx < TOTAL_SLAVES; slave_idx++) {
        ...
        for (hist_idx = 0; hist_idx < RECORDS_PER_SLAVE; hist_idx++) {
            ...
            if (hist_rec->is_valid && hist_rec->temp > 0) {
                // 假设timestamp包含日期信息，这里简化为直接判断（实际需根据timestamp解析日期）
                current_temp = hist_rec->temp * 10;
                
                // 若当前温度高于当日已记录最高温，更新当日数据
