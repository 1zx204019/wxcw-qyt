...
#define NULL ((void*)0)
	...
	#ifndef MAX_TX_COUNT
 #define MAX_TX_COUNT TOTAL_SLAVES // 使用 TOTAL_SLAVES 的值
#endif
// ------------------- 全局变量定义 -------------------
...
static void UpdateDisplayForPage4(void);
...
static void DisplayPage4(void) {
    ...
    unsigned char row;
    DataRecord* dev_data;
    DataRecord* check_dev_data;
    unsigned char status_str[STATUS_STR_LEN];
    unsigned char pos;
    unsigned char tx_idx;
    unsigned char check_aid;
    bit is_abnormal;
    
    LCD_Clear();

    // 2. 遍历显示最多 MAX_PID_DISPLAY 个 PID 的状态
    for (pid = 1; pid <= MAX_PID_DISPLAY; pid++) {
        row = PID_ROW_START + (pid - 1); // 计算当前行号
        
        // 3. 初始化状态字符串为0
        for (pos = 0; pos < STATUS_STR_LEN; pos++) {
            status_str[pos] = 0;
        }
        pos = 0;
        
        // 4. 获取该 PID 对应的设备数据
        dev_data = GetRecentDataByAID(pid); // AID == PID

        // 5. 显示 PID 标签和数值
        LCD_DisplayString(row, PID_COL, "PID=");
        if (dev_data != 0 && dev_data->is_valid != 0) {
            // 显示PID值
            LCD_DisplayNumber(row, PID_COL + 4, (unsigned long)dev_data->pid, 2);
        } else {
            LCD_DisplayString(row, PID_COL + 4, "--");
        }
        ...
        if (dev_data != 0 && dev_data->is_valid != 0) {
            ...
            if (is_abnormal != 0) {
                ...
                for (tx_idx = 0; tx_idx < MAX_TX_COUNT; tx_idx++) {
                    // 防止数组越界（预留4个字符给"TXn "）
                    if (pos >= STATUS_STR_LEN - 4) break;
                    
                    check_aid = tx_idx + 1;
                    check_dev_data = GetRecentDataByAID(check_aid);

                    // 检查设备数据有效性和温度阈值
                    if (check_dev_data != 0 && 
                        check_dev_data->is_valid != 0 && 
                        check_dev_data->temp > 25) {
                        // 拼接 "TXn " 到状态字符串
                        status_str[pos++] = 'T';
                        status_str[pos++] = 'X';
                        if (tx_idx < 9) {
                            status_str[pos++] = '0' + (tx_idx + 1);
                        } else {
                            // 两位数的处理
                            status_str[pos++] = '0' + ((tx_idx + 1) / 10);
                            status_str[pos++] = '0' + ((tx_idx + 1) % 10);
                        }
                        status_str[pos++] = ' ';
                    }
            ...
            // 显示状态字符串
            LCD_DisplayString(row, STATUS_COL, status_str);
        } else {
            // 无有效数据时显示"--"
            LCD_DisplayString(row, STATUS_COL, "--");
        }
    }

    // 7. 显示底部按钮（使用ASCII字符）
    LCD_DisplayString(4, 0, "Prev");
    LCD_DisplayString(4, 32, "Next");
    LCD_DisplayString(4, 64, "Back");
}
...
unsigned char module_index = menu_state.page3_selected; // 当前选中的模块索引
    ...
    if (dev_data != NULL && dev_data->is_valid) {
        ...
        if (temp_display == 0) {
            ...
            LCD_DisplayNumber(2, 24, (unsigned long)temp_display, 2);
        }
    } else {
        LCD_DisplayString(2, 24, (unsigned char*)"--");
    }
    ...
    if (dev_data != NULL && dev_data->is_valid) {
        unsigned int volt_mv = dev_data->volt1 * 100;  // 转换为mV
        LCD_DisplayNumber(4, 24, (unsigned long)volt_mv, 4);
    } else {
        LCD_DisplayString(4, 24, (unsigned char*)"----");
    }
}
...
// --- PAGE_4 局部刷新函数 (修复版) ---
static void UpdateDisplayForPage4(void) {
    unsigned char row = 0; // 第一个PID行（可根据实际需求扩展多行）
    DataRecord* dev_data = GetRecentDataByAID(1); // 获取AID=1的设备数据
    unsigned char status_str[16]; // 状态字符串缓冲区（仅定义一次）
    #define STATUS_COL 32 // 补充缺失的宏定义（状态显示起始列）
    
    // 更新PID数值显示
    if (dev_data != NULL && dev_data->is_valid) {
        LCD_DisplayNumber(row, 4, (unsigned long)dev_data->pid, 2);
    } else {
        LCD_DisplayString(row, 4, (unsigned char*)"--");
    }
    
    // 更新状态显示（温度>25℃判定异常）
    if (dev_data != NULL && dev_data->is_valid) {
        if (dev_data->temp > 25) {
            sprintf((char*)status_str, "异常：TX%02d", dev_data->aid); // 显示异常设备
        } else {
            sprintf((char*)status_str, "正常");
        }
        LCD_DisplayString(row, STATUS_COL, status_str);
    } else {
        LCD_DisplayString(row, STATUS_COL, (unsigned char*)"--");
    }
}
