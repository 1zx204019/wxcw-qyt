...
#define MAX_MAX_TEMP_EVENTS  3  // 新增：最高温事件最大数量（和报警事件一致）
...
// uart4.h - 在宏定义部分添加
#define HISTORY_SAVE_INTERVAL 300000  // 保存间隔：2分钟（120秒 × 1000ms）

#define PAGE19_READ_INTERVAL 300000                   // 5分钟 = 300秒 × 1000ms
sbit CE = P5^4;
...
//extern short max_temps[3];                // PAGE21显示用的温度缓存数组
//extern rtc_time_t max_temp_times[3];      // PAGE21显示用的时间缓存数组
...
//void ClearPage22HistoryTempData(unsigned char group);
//void ClearPage21MaxTempData(unsigned char line);
