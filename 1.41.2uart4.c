...
static void Page19_AutoReadHistoryData(void) {
    ...
    // 核心1：独立触发间隔控制，1秒仅执行一次
    current_time = GetSystemTick();
    if (current_time - page19_trigger_timer < PAGE19_TRIGGER_INTERVAL) {
        return;
    }
    page19_trigger_timer = current_time;  // 更新触发时间戳（仅赋值一次）
    
    // 核心2：保留当前时间戳，统一用于5分钟间隔判断
    // 不再重新赋值current_time，避免时间差导致误判
    for (i = 0; i < 3; i++) {
        aid = i + 1;
        // 核心3：添加边界保护，确保时间差计算不溢出（针对unsigned long特性）
        if (page19_last_read_time[i] == 0 || 
            (current_time >= page19_last_read_time[i] && (current_time - page19_last_read_time[i]) >= PAGE19_READ_INTERVAL) ||
            (current_time < page19_last_read_time[i])) {  // 处理system_tick溢出情况
            ...
            if (realtime_data != NULL && realtime_data->is_valid) {
                ...
                page19_auto_records[i][idx].record_time = read_time;
                page19_auto_records[i][idx].is_valid = 1;
                
                page19_record_index[i] = (idx + 1) % 4;
                
                // 调试信息（新增时间差打印，便于验证）
                UART4_SendString("Page19 Record - AID=");
                UART4_SendNumber(aid, 1);
                UART4_SendString(", Time=");
                UART4_SendNumber(read_time.hour, 2);
                UART4_SendString(":");
                UART4_SendNumber(read_time.min, 2);
                UART4_SendString(", Interval=");
                UART4_SendNumber(current_time - page19_last_read_time[i], 6);
                UART4_SendString("ms\r\n");
            }
        }
    }
...
static void DisplayPage19(void) {
    ...
    unsigned char target_idx = page19_selected_record; // 使用正确的索引
    Page19_AutoRecord* auto_record = NULL;
    ...
    // 读取选中的自动记录（来自 page19_auto_records）
    if (curr_slave < 3 && target_idx < 4) {
        auto_record = &page19_auto_records[curr_slave][target_idx];
    }
    ...
    LCD_DISPLAYCHAR_NEW(0, 16, 0, 15); // ":"
    if (auto_record != NULL && auto_record->is_valid) {
        LCD_DisplayNumber(0, 24, (unsigned long)auto_record->temp, 2); // 温度值（如40）
    } else {
        LCD_DisplayNumber(0, 24, 0UL, 2); // 无数据时显示00
    }
    ...
    if (auto_record != NULL && auto_record->is_valid) {
        LCD_DisplayNumber(0, 80, (unsigned long)auto_record->volt_mv, 4); // 电压值（如2000）
    } else {
    ...
    LCD_DISPLAYCHAR_NEW(2, 16, 0, 15); // ":"
    // 优先显示自动记录的时间，无则显示当前时间
    if (auto_record != NULL && auto_record->is_valid) {
        LCD_DisplayNumber(2, 24, (unsigned long)auto_record->record_time.mon, 2);  // 月
        LCD_DISPLAYCHAR_NEW(2, 40, 1, 15); // "."
        LCD_DisplayNumber(2, 48, (unsigned long)auto_record->record_time.day, 2);   // 日
    } else {
        LCD_DisplayNumber(2, 24, (unsigned long)curr_time.mon, 2);  // 当前月（如08）
        LCD_DISPLAYCHAR_NEW(2, 40, 1, 15); // "."
        LCD_DisplayNumber(2, 48, (unsigned long)curr_time.day, 2);   // 当前日（如04）
    }
    ...
    // 优先显示自动记录的时间，无则显示当前时间
    if (auto_record != NULL && auto_record->is_valid) {
        LCD_DisplayNumber(4, 24, (unsigned long)auto_record->record_time.hour, 2); // 时
        LCD_DISPLAYCHAR_NEW(4, 40, 0, 15); // ":"
        LCD_DisplayNumber(4, 48, (unsigned long)auto_record->record_time.min, 2);  // 分
    } else {
        LCD_DisplayNumber(4, 24, (unsigned long)curr_time.hour, 2); // 当前时（如16）
        LCD_DISPLAYCHAR_NEW(4, 40, 0, 15); // ":"
        LCD_DisplayNumber(4, 48, (unsigned long)curr_time.min, 2);  // 当前分（如20）
    }
...
static void DeletePage19SelectedRecord(void) {
    ...
    if (curr_slave < 3 && target_idx < 4) {
        // 1. 标记为无效
        page19_auto_records[curr_slave][target_idx].is_valid = 0;
        
        // 2. 查找下一条有效记录
       
        for (i = 1; i <= 4; i++) {
            unsigned char next_idx = (target_idx + i) % 4;
            if (page19_auto_records[curr_slave][next_idx].is_valid) {
