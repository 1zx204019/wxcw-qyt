...
static void Page19_AutoReadHistoryData(void);
// ------------------- 设备列表定义 -------------------
...
static void Page19_AutoReadHistoryData(void) {
    ...
    // 只处理前3个从站（TX01-TX03）
    for (i = 0; i < 3; i++) {
        unsigned char aid = i + 1;  // AID从1开始
        
        // 检查是否达到5分钟间隔
        if (page19_last_read_time[i] == 0 || 
            (current_time - page19_last_read_time[i]) >= PAGE19_READ_INTERVAL) {
            
            // 更新时间戳
            page19_last_read_time[i] = current_time;
            
            // 获取当前时间作为记录时间
            GetCurrentRTC();
            read_time = current_rtc_time;
            
            // 优先读取实时数据
            realtime_data = GetRecentDataByAID(aid);
            
            if (realtime_data != NULL && realtime_data->is_valid) {
                // 存储到自动记录数组中
                unsigned char idx = page19_record_index[i];
                
                page19_auto_records[i][idx].pid = realtime_data->pid;
                page19_auto_records[i][idx].aid = aid;
                page19_auto_records[i][idx].temp = realtime_data->temp;
                page19_auto_records[i][idx].volt_mv = realtime_data->volt1 * 100;
                page19_auto_records[i][idx].record_time = read_time;  // 存储读取时的时间
                page19_auto_records[i][idx].is_valid = 1;  // 改为 1 而不是 bit
            
                // 更新索引（循环覆盖，0-3）
                page19_record_index[i] = (idx + 1) % 4;
                
                // 调试信息
                UART4_SendString("Page19 Auto Record - AID=");
                UART4_SendNumber(aid, 1);
                UART4_SendString(", Time=");
                UART4_SendNumber(read_time.hour, 2);
                UART4_SendString(":");
                UART4_SendNumber(read_time.min, 2);
                UART4_SendString("\r\n");
            }
        }
    }
...
void AddDataToSummary(unsigned char pid, unsigned char aid, 
    ...
    if (last_save_time[slave_idx] == 0 || 
        (current_time - last_save_time[slave_idx]) >= HISTORY_SAVE_INTERVAL) {
        ...
        if (recent_rec->is_valid) {
            ...
            // 关键：获取并保存当前的RTC时间
            GetCurrentRTC();  // 更新 current_rtc_time
...
static void DisplayPage19(void) {
    unsigned char curr_slave = menu_state.page18_selected;
    unsigned char aid = curr_slave + 1;
    unsigned short hist_start = HISTORY_DATA_START + (curr_slave * RECORDS_PER_SLAVE);
    unsigned char hist_idx = page18_hist_index[curr_slave];
    
    // 修改1：根据 page19_selected_record 计算显示哪条记录
    // 0=最新记录，1=上一条，2=上两条，3=上三条
    unsigned char target_idx = (hist_idx - page19_selected_record + RECORDS_PER_SLAVE) % RECORDS_PER_SLAVE;
    unsigned short target_pos = hist_start + target_idx;
    
    DataRecord* dev_data = NULL;
    rtc_time_t record_time;  // 历史记录的保存时间
    ...
    // 清空显示区域
    LCD_DisplayString(0, 0, (unsigned char*)"                ");
    LCD_DisplayString(2, 0, (unsigned char*)"                ");
    LCD_DisplayString(4, 0, (unsigned char*)"                ");

    // 修改2：获取指定记录的数据（基于 page19_selected_record）
    if (target_pos < TOTAL_RECORDS && data_summary[target_pos].is_valid) {
        dev_data = &data_summary[target_pos];
        record_time = dev_data->save_time;
    }

    // 删除箭头显示部分（保持原样，不显示箭头）
    // 原来的箭头代码删掉，只清空那个位置
    LCD_DisplayChar(0, 0, ' ');  // 清空箭头位置

    // 1. 第一行：温度 + 电压（固定字符保持不变）
    LCD_DISPLAYCHAR_NEW(0, 8, 0, 1);  // "温"
    LCD_DISPLAYCHAR_NEW(0, 16, 1, 1); // "度"
    LCD_DISPLAYCHAR_NEW(0, 24, 0, 15); // ":"
    if (dev_data != NULL && dev_data->is_valid) {
        LCD_DisplayNumber(0, 32, (unsigned long)dev_data->temp, 2);
    } else {
        LCD_DisplayNumber(0, 32, 0UL, 2);
    }
    LCD_DISPLAYCHAR_NEW(0, 48, 0, 2);  // "摄氏度"

    LCD_DISPLAYCHAR_NEW(0, 64, 0, 3);  // "电"
    LCD_DISPLAYCHAR_NEW(0, 72, 1, 3);  // "压"
    LCD_DISPLAYCHAR_NEW(0, 80, 0, 15); // ":"
    if (dev_data != NULL) {
        LCD_DisplayNumber(0, 88, (unsigned long)(dev_data->volt1 * 100), 4);
    } else {
        LCD_DisplayNumber(0, 88, 0UL, 4);
    }
    LCD_DisplayString(0, 120, (unsigned char*)"mV");

    // 2. 第二行：日期（格式：MM.DD）- 显示保存日期
    LCD_DISPLAYCHAR_NEW(2, 8, 2, 12);  // "日"
    LCD_DISPLAYCHAR_NEW(2, 16, 3, 12); // "期"
    LCD_DISPLAYCHAR_NEW(2, 24, 0, 15); // ":"
    if (dev_data != NULL && dev_data->is_valid) {
        LCD_DisplayNumber(2, 32, (unsigned long)record_time.mon, 2);
        LCD_DISPLAYCHAR_NEW(2, 48, 1, 15); // "."
        LCD_DisplayNumber(2, 56, (unsigned long)record_time.day, 2);
    } else {
        GetCurrentRTC();
        LCD_DisplayNumber(2, 32, (unsigned long)current_rtc_time.mon, 2);
        LCD_DISPLAYCHAR_NEW(2, 48, 1, 15);
        LCD_DisplayNumber(2, 56, (unsigned long)current_rtc_time.day, 2);
    }

    // 3. 第三行：时间（格式：HH:MM）- 显示保存时间
    LCD_DISPLAYCHAR_NEW(4, 8, 2, 20);  // "时"
    LCD_DISPLAYCHAR_NEW(4, 16, 3, 20); // "间"
    LCD_DISPLAYCHAR_NEW(4, 24, 0, 15); // ":"
    if (dev_data != NULL && dev_data->is_valid) {
        LCD_DisplayNumber(4, 32, (unsigned long)record_time.hour, 2);
        LCD_DISPLAYCHAR_NEW(4, 48, 0, 15); // ":"
        LCD_DisplayNumber(4, 56, (unsigned long)record_time.min, 2);
    } else {
        GetCurrentRTC();
        LCD_DisplayNumber(4, 32, (unsigned long)current_rtc_time.hour, 2);
        LCD_DISPLAYCHAR_NEW(4, 48, 0, 15);
        LCD_DisplayNumber(4, 56, (unsigned long)current_rtc_time.min, 2);
    }
...
unsigned short next_pos;     // 1. 将选中的记录标记为无效
	unsigned short pos;
	 unsigned char valid_count;
    if (target_pos < TOTAL_RECORDS) {
        data_summary[target_pos].is_valid = 0;
    }

    // 2. 将后续有效记录向前移动（覆盖被删除的记录）
    for (i = target_idx; i < RECORDS_PER_SLAVE - 1; i++) {
       curr_pos = hist_start + i;
       next_pos = hist_start + i + 1;
        
        if (next_pos < TOTAL_RECORDS && data_summary[next_pos].is_valid) {
            // 复制下一条记录到当前位置
            data_summary[curr_pos] = data_summary[next_pos];
            data_summary[next_pos].is_valid = 0;
        }
    }
    ...
    for (i = 0; i < RECORDS_PER_SLAVE; i++) {
         pos = hist_start + i;
        if (data_summary[pos].is_valid) {
            page18_hist_index[curr_slave] = i;
        }
    }

    // 4. 更新 page19_selected_record，确保显示下一条有效数据
    // 关键修改：删除后保持显示当前位置
    // 因为数据已经前移，当前位置现在就是下一条数据
    // 如果删除的是最后一条有效数据，则重置为0
   valid_count = 0;
    for (i = 0; i < RECORDS_PER_SLAVE; i++) {
        pos = hist_start + i;
        if (data_summary[pos].is_valid) {
    ...
    } else if (page19_selected_record >= valid_count) {
        // 删除后当前索引超出有效数据范围，显示第一条
        page19_selected_record = 0;
    }
    // 否则保持 page19_selected_record 不变，因为数据已经前移

    UART4_SendString("PAGE19 record deleted.\r\n");
}
...
void LCD_HandleKey(unsigned char key) {
    ...
    //                display_labels_initialized = 0;
                ...
                display_labels_initialized = 0;
							  // 初始化选中记录为第一条
    page19_selected_record = 0;
    page19_selected_slave = menu_state.page18_selected;
    return;
            } else if (current_page == PAGE_19) {
                // 从清除确认页面进入删除确认页面
...
void DeleteMaxTempEvent(unsigned char display_index) {
    ...
    // 1. 彻底清空 daily_max_temps[display_index]
