...
// 新增：最高温事件变量（对应头文件声明）
//DailyMaxTemp daily_max_temps[MAX_MAX_TEMP_EVENTS] = {0};
unsigned char max_temp_event_count = 0;
unsigned char max_temp_next_index = 0;
bit disable_max_temp_calc = 0; // 禁用最高温计算标记

// 全局变量区新增（与 daily_max_temps 等变量同级）
short max_temps[3] = {-990, -990, -990}; // 初始化无效值，避免脏数据
rtc_time_t max_temp_times[3] = {0};      // 时间数组初始化
// 原有变量保留，无需修改定义
...
// 近3天最高温存储数组
DailyMaxTemp daily_max_temps[3] = {0};
// 记录当前系统日期（用于判断是否跨天）
unsigned char current_system_date[3] = {0}; // 年、月、日
// ------------------- 静态函数声明 -------------------
...
static void DisplayPage21(void) {
    ...
    int i; // 仅保留 i 的定义，删除 max_temps 和 max_temp_times 的局部声明
    
    GetMaxTempRecords(max_temps, max_temp_times); // 直接使用全局变量
    LCD_Clear();
    ...
    for (i = 0; i < 3; i++) {
        ...
        // 关键修改：双重校验（缓存有效 + 数据源有效）
        if (max_temps[i] != -990 && daily_max_temps[i].is_valid && daily_max_temps[i].max_temp != -990) {
            // 温度
...
static void DisplayPage22(void) {
    ...
    short max_temps[3];              // 前3高温度（放大10倍）
    rtc_time_t max_temp_times[3];    // 对应时间戳
    unsigned char selected_row = menu_state.page21_selected; // 关联 PAGE_21 选中行（0=第1高，1=第2高，2=第3高）
    unsigned char pid = 0, aid = 0;
    unsigned short volt_mv = 0;
    unsigned char temp = 0;
    DataRecord *target_record = NULL;
    
    // 2. 读取最高温记录及对应 PID/AID/电压
    GetMaxTempRecords_Ext(max_temps, max_temp_times, &pid, &aid, &volt_mv, &target_record);
    
    // 3. 清屏并绘制详情页固定标签（完全复用 PAGE_11 布局）- 固定字符不变
    LCD_Clear();
    ...
    // 关键：判断数据是否已清空（max_temps=-990 表示清空）
   // 关键：判断数据是否已清空（max_temps=-990 表示清空）
// 原显示逻辑替换为：
if (max_temps[selected_row] != -990 && daily_max_temps[selected_row].is_valid && daily_max_temps[selected_row].max_temp != -990) {
    temp = (unsigned char)(max_temps[selected_row] / 10);
    LCD_DisplayNumber(0, 56, pid, 2);
    LCD_DisplayNumber(0, 112, aid, 2);
    LCD_DisplayNumber(2, 24, temp, 2);
    LCD_DISPLAYCHAR_NEW(2, 40, 0, 2);
    LCD_DisplayNumber(2, 88, max_temp_times[selected_row].mon, 2);
    LCD_DISPLAYCHAR_NEW(2, 104, 1, 15);
    LCD_DisplayNumber(2, 112, max_temp_times[selected_row].day, 2);
    LCD_DisplayNumber(4, 24, volt_mv, 4);
} else {
    LCD_DisplayString(0, 56, (unsigned char*)"--");
    LCD_DisplayString(0, 112, (unsigned char*)"--");
...
unsigned char actual_index = menu_state.page21_selected;
	unsigned char i;
    if (actual_index < 3) {
        // 1. 清空原始数据源（强化校验）
        daily_max_temps[actual_index].is_valid = 0;
        daily_max_temps[actual_index].max_temp = -990;
        daily_max_temps[actual_index].pid = 0;
        daily_max_temps[actual_index].aid = 0;
        daily_max_temps[actual_index].volt_mv = 0;
        memset(&daily_max_temps[actual_index].temp_time, 0, sizeof(rtc_time_t));
            ...
            if (current_abnormal == 1) {
                ...
                for (j = 0; j < alarm_event_count; j++) {
                    ...
                    if (alarm_events[alarm_idx].is_valid && alarm_events[alarm_idx].aid == (i + 1)) {
                        // 调用恢复事件记录函数（需确保RecordRecoveryEvent已声明）
                        RecordRecoveryEvent(
                            dev_data->pid,        // 从站ID
                            i + 1,                // 区域AID
    ...
    *target_record = NULL;
    ...
    for (i = 0; i < 3; i++) {
        if (daily_max_temps[i].is_valid && daily_max_temps[i].max_temp != -990) {
            ...
            DataRecord *realtime_rec = GetRecentDataByAID(i + 1);
            if (realtime_rec != NULL && realtime_rec->is_valid && realtime_rec->temp > 0) {
                max_temps[i] = realtime_rec->temp * 10;
...
static void CheckDailyMaxTemp(void) {
    ...
    DataRecord *realtime_rec = NULL; // 实时数据指针
    ...
    // 新增：优先读取实时从站数据（data_summary[0-TOTAL_SLAVES-1] 存储实时数据）
    for (slave_idx = 0; slave_idx < TOTAL_SLAVES; slave_idx++) {
        realtime_rec = &data_summary[slave_idx]; // 实时数据存储位置
        if (realtime_rec->is_valid && realtime_rec->temp > 0) { // 实时数据有效
            current_temp = realtime_rec->temp * 10; // 放大10倍，与结构体定义一致
            
            // 若当前温度高于当日已记录最高温，更新 daily_max_temps[0]（当天数据）
            if (current_temp > daily_max_temps[0].max_temp) {
                daily_max_temps[0].max_temp = current_temp;
                rtc_read(&daily_max_temps[0].temp_time); // 记录当前时间
                daily_max_temps[0].pid = realtime_rec->pid; // 实时PID
                daily_max_temps[0].aid = realtime_rec->aid; // 实时AID
                daily_max_temps[0].volt_mv = realtime_rec->volt1 * 100; // 实时电压（mV）
                daily_max_temps[0].is_valid = 1; // 标记为有效（关键）
            }
        }
    }
    
    // 保留原有历史数据遍历（补充实时数据的遗漏场景）
    for (slave_idx = 0; slave_idx < TOTAL_SLAVES; slave_idx++) {
        hist_start = HISTORY_DATA_START + (slave_idx * RECORDS_PER_SLAVE);
...
void DeleteMaxTempEvent(unsigned char display_index) {
    ...
    max_temp_event_count = 0;
    for (i = 0; i < 3; i++) {
        if (daily_max_temps[i].is_valid) {
            max_temp_event_count++;
        }
    }
    
    UART4_SendString("Max temp event deleted (marked invalid).\r\n");
}
