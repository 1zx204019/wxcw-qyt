...
static void DisplayPage19(void) {
    ...
    LCD_DISPLAYCHAR_NEW(0, 16, 0, 15); // ":"
    if (dev_data != NULL && dev_data->is_valid) {
        LCD_DisplayNumber(0, 24, (unsigned long)dev_data->temp, 2);
    } else {
    ...
    if (dev_data != NULL) {
        LCD_DisplayNumber(0, 80, (unsigned long)(dev_data->volt1 * 100), 4);
    } else {
    ...
    if (dev_data != NULL && dev_data->is_valid) {
        // 显示保存的月份和日期
        LCD_DisplayNumber(2, 24, (unsigned long)record_time.mon, 2);
        LCD_DISPLAYCHAR_NEW(2, 40, 1, 15); // "."
        LCD_DisplayNumber(2, 48, (unsigned long)record_time.day, 2);
    } else {
        // 无数据时显示当前日期
        GetCurrentRTC();
        LCD_DisplayNumber(2, 24, (unsigned long)current_rtc_time.mon, 2);
        LCD_DISPLAYCHAR_NEW(2, 40, 1, 15);
        LCD_DisplayNumber(2, 48, (unsigned long)current_rtc_time.day, 2);
    }
    ...
    if (dev_data != NULL && dev_data->is_valid) {
        // 显示保存的小时和分钟
        LCD_DisplayNumber(4, 24, (unsigned long)record_time.hour, 2);
        LCD_DISPLAYCHAR_NEW(4, 40, 0, 15); // ":"
        LCD_DisplayNumber(4, 48, (unsigned long)record_time.min, 2);
    } else {
        // 无数据时显示当前时间
        GetCurrentRTC();
        LCD_DisplayNumber(4, 24, (unsigned long)current_rtc_time.hour, 2);
        LCD_DISPLAYCHAR_NEW(4, 40, 0, 15);
        LCD_DisplayNumber(4, 48, (unsigned long)current_rtc_time.min, 2);
    }
