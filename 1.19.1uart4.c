...
unsigned char page10_scroll_page = 0;  // PAGE10分页控制（0=第1页，1=第2页）
// 恢复事件相关
...
void AddDataToSummary(unsigned char pid, unsigned char aid, 
    ...
    // ========== 优化核心：仅在PAGE_1时刷新显示 ==========
    if (menu_state.current_page == PAGE_1) { // 新增页面判断，仅PAGE_1执行刷新
        
        for (i = 0; i < 2; i++) {
            unsigned char dev_idx = page1_scroll_idx + i;  // 当前PAGE_1显示的2个从站索引
            unsigned char row = (i == 0) ? 2 : 4;          // 固定显示行（第2行、第4行）
            Page1_DevInfo* dev = &page1_devices[dev_idx];
            
            // 匹配当前更新数据的从站AID，仅更新对应显示项
            if (dev->aid == aid) {
                // 刷新设备ID（确保ID与数据对应）
                dev_id_buf[0] = '0';
                dev_id_buf[1] = '1';
...
static void DisplayPage10(void) {
    ...
    if (display_labels_initialized == 0 || menu_state.page_changed) {
        ...
        for (i = 0; i < PAGE10_DISPLAY_COUNT; i++) {
            ...
            LCD_DisplayChar(row, 24, '1' + (page10_scroll_page * PAGE10_DISPLAY_COUNT + i));  // 事件编号（1-6）
...
void LCD_HandleKey(unsigned char key) {
    ...
    switch(key) {
     ...
     if (current_page == PAGE_1) {
        if (page1_scroll_idx > 0) {
            page1_scroll_idx--;  // 索引减1，显示前一组（如TX03-TX04→TX02-TX03）
        } else {
            page1_scroll_idx = PAGE1_TOTAL_DEVICES - 2;  // 循环到最后一组（TX29-TX30）
        }
        display_labels_initialized = 0;
    ...
    menu_state.page_changed = 1;
    display_labels_initialized = 0;
		 RefreshDisplay(); // 新增：强制刷新页面，立即显示下一条
    return;
        ...
        return;
                ...
                HandlePrevItem();
                return;  // 仅刷新箭头，无需后续RefreshDisplay
