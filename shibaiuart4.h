...
#define PAGE3_MAX_MODULES 10 // 假设总共可能有 TOTAL_SLAVES 个模块，或者你定义一个具体的数量，比如 2 个
...
#define MAX_RECOVERY_EVENTS 3  // 最多记录3个恢复事件（与报警事件一致）
...
// EEPROM配置（1K字节，地址范围0x000-0x3FF）
#define EEPROM_MAX_DAYS 4       // 存储最近4天最高温
#define EEPROM_ADDR_BASE 0xFE0400UL  // 加UL表示无符号长整数，避免地址溢出
#define EEPROM_RECORD_SIZE 16   // 每条记录占用字节数（PID+AID+温度+电压+日期时间）
sbit CE = P5^4;
...
// uart4.h 开头添加（PageType 枚举之后）
// uart4.h 中修改结构体声明（删除多余的struct关键字，直接typedef）
typedef struct {
    unsigned char pid;           // 从站ID（1字节）
    unsigned char aid;           // 区域ID（1字节）
    unsigned char temp;          // 最高温度（℃，1字节）
    unsigned int volt_mv;        // 电压（mV，2字节）
    rtc_time_t time;            // 日期时间（6字节）
    unsigned char is_valid;      // 记录有效性（1字节）
    unsigned char reserved[4];   // 预留字节（凑16字节）
} EEPROM_MaxTempRecord; // 直接typedef，使用时无需加struct
// 菜单项枚举
typedef enum {
    MENU_MEASURE_DATA = 0,
...
// uart4.h 中函数声明（无struct关键字）
void EEPROM_ReadMaxTempRecords(EEPROM_MaxTempRecord records[]);
void EEPROM_SaveTodayMaxTemp(EEPROM_MaxTempRecord *new_rec);
void EEPROM_DeleteMaxTempRecord(unsigned char day_idx);
