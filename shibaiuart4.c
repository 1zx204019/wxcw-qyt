...
// PAGE_1滚动控制：当前显示的起始索引（0-8，对应显示TX1-TX2到TX9-TX10）
unsigned char page1_scroll_idx = 0;
...
static unsigned int pwd_flash_timer = 0; // 改为：单次闪烁计时器（计数250ms）
static bit pwd_flash_state = 1;          // 改为：闪烁触发标记（1=正在闪烁，0=已结束）


        static unsigned int flash_timer = 0;
        static bit flash_state = 1;

// 新增：定义临时密码存储（对应PAGE_24的 edit_temp_time）
static unsigned char edit_temp_pwd[6] = {0};
// 移除原有的 PWD_FLASH_INTERVAL 定义，复用 FLASH_INTERVAL
...
// 显示从站设备列表页面（PAGE_1）
// 功能：展示2个从站（01TX01/01TX02）的实时温度（整数）和电压（mV单位），支持上下键选择设备，提供页面切换和返回功能
static void DisplayPage1(void); 

// 显示主功能菜单页面（PAGE_2）
// 功能：展示测量数据、传感器状态、参数查询等5个功能选项，支持上下滚动选择，提供进入子菜单和返回主页面的操作
static void DisplayPage2(void);

// 显示测量数据详情页面（PAGE_3）
// 功能：展示选中模块的PID、TX编号、实时温度和电压（mV），支持上下切换模块，提供页面切换和返回功能
static void DisplayPage3(void);

// 显示传感器异常状态页面（PAGE_4）
// 功能：筛选并显示温度>25℃的异常传感器，展示其PID、异常标识和TX编号，最多显示3个异常设备，支持页面切换和返回
static void DisplayPage4(void);

// 显示历史记录查询子菜单页面（PAGE_7）
// 功能：展示温度报警、预警、恢复事件、历史数据、最高温度查询5个选项，支持上下选择，提供进入子菜单和返回功能
static void DisplayPage7(void);

// 显示设备维护菜单页面（PAGE_8）
// 功能：展示修改日期时间、修改密码、恢复出厂设置3个选项，支持上下选择，提供进入子菜单和返回功能
static void DisplayPage8(void);

// 显示温度报警事件列表页面（PAGE_10）
// 功能：展示最近3条报警事件的编号和发生时间，支持上下翻页、进入详情页和返回，动态刷新报警数据
static void DisplayPage10(void);

// 显示报警事件详细数据页面（PAGE_11）
// 功能：展示选中报警事件的PID、AID、温度、电压（mV），支持删除当前报警和返回列表页
static void DisplayPage11(void);

// 显示报警事件删除确认页面（PAGE_12）
// 功能：展示报警删除确认提示，支持确认删除（返回列表页）和取消返回（返回详情页）
static void DisplayPage12(void);

// 显示温度预警事件列表页面（PAGE_13）
// 功能：展示最近3条预警事件的编号和发生时间，支持上下翻页、进入详情页和返回，结构与报警列表一致
static void DisplayPage13(void);

// 显示传感器恢复事件列表页面（PAGE_14）
// 功能：展示最近3条恢复事件的编号和恢复时间，支持上下翻页、进入详情页和返回，动态刷新恢复数据
static void DisplayPage14(void);

// 显示预警/恢复事件详细数据页面（PAGE_15）
// 功能：展示选中事件的PID、AID、恢复前异常温度和恢复后温度，支持删除当前事件和返回列表页
static void DisplayPage15(void);

// 显示预警事件删除确认页面（PAGE_16）
// 功能：展示预警删除确认提示，支持确认删除（返回列表页）和取消返回（返回详情页）
static void DisplayPage16(void);

// 显示恢复事件删除确认页面（PAGE_17）
// 功能：展示恢复事件删除确认提示，支持确认删除（返回列表页）和取消返回（返回详情页）
static void DisplayPage17(void);

// 显示历史数据查询页面（PAGE_18）
// 功能：展示多个从站的PID和TX编号，支持上下选择从站、切换查看历史数据项，提供详情查看和返回功能
static void DisplayPage18(void);

// 显示清除历史数据确认页面（PAGE_19）
// 功能：展示选中从站的历史数据（温度、电压、日期时间），支持上下切换历史项，提供清除确认和返回功能
static void DisplayPage19(void);

// 显示历史数据删除确认页面（PAGE_20）
// 功能：展示历史数据删除最终确认提示，支持确认删除（返回查询页）和取消返回（返回确认页）
static void DisplayPage20(void);

static void EEPROM_ReadMaxTempRecords(EEPROM_MaxTempRecord records[]);
static void EEPROM_SaveTodayMaxTemp(EEPROM_MaxTempRecord *new_rec);
static void EEPROM_DeleteMaxTempRecord(unsigned char day_idx);
// 显示最高温度查询页面（PAGE_21）
// 功能：展示前3高温度及对应日期时间（月-日 时:分），选中项闪烁，支持上下选择、进入详情页和返回
static void DisplayPage21(void);

// 显示最高温度清除确认页面（PAGE_22）
// 功能：展示选中最高温事件的PID、AID、温度、电压和发生时间，支持删除确认和返回查询页
static void DisplayPage22(void);

// 显示最高温度删除确认页面（PAGE_23）
// 功能：展示最高温数据删除最终确认提示，支持确认删除（返回详情页）和取消返回（返回确认页）
static void DisplayPage23(void);


// 显示修改日期时间页面（PAGE_24）
// 功能：展示当前日期（YYYY-MM-DD）和时间（HH:MM:SS），支持选择修改位置（年/月/日/时/分）、调整数字，提供保存和返回功能
static void DisplayPage24(void);

// 显示修改密码页面（PAGE_25）
// 功能：展示6位密码（选中位闪烁），支持选择密码位、调整数字（0-9循环），提供保存修改和返回功能
static void DisplayPage25(void);

// 显示恢复出厂设置确认页面（PAGE_26）
// 功能：展示恢复出厂设置确认提示，支持确认恢复（返回维护菜单）和取消返回（返回维护菜单）
...
static void GetMaxTempRecords_Ext(short max_temps[], rtc_time_t max_temp_times[], 
                                unsigned char *pid, unsigned char *aid, 
                                unsigned short *volt_mv, DataRecord **target_record);
...
static void PWD_Edit_Init(void); // 初始化密码编辑
static void PWD_Switch_Pos(signed char step); // 切换密码位
static void PWD_Adjust_Num(signed char step); // 调整数字
static void PWD_Save_Edit(void); // 保存密码修改
static void DisplayPassword(void); // 显示密码（含闪烁逻辑）
...
static void DisplayPage1(void) {
    ...
    // 清空数据显示区域（仅第2行、第4行，屏幕支持的有效行）
    for (i = 0; i < 2; i++) {
        unsigned char row = (i == 0) ? 2 : 4;  // 固定行：第一个从站→2行，第二个→4行
...
static void DisplayPage18(void) {
    ...
    unsigned char i;
    DataRecord* dev_data = NULL;
    unsigned char curr_slave = menu_state.page18_selected; // 当前选中的从站（0=TX01，1=TX02，2=TX03）
    unsigned char aid = curr_slave + 1; // 从站AID（1-3）
    unsigned short hist_start = HISTORY_DATA_START + (curr_slave * RECORDS_PER_SLAVE);
...
static void DisplayPage19(void) {
    // 第一步：所有局部变量集中声明在函数开头（C89标准）
    unsigned char curr_slave = menu_state.page18_selected; // 与page18选中的从站联动
    unsigned char aid = curr_slave + 1;
    unsigned short hist_start = HISTORY_DATA_START + (curr_slave * RECORDS_PER_SLAVE);
    unsigned char hist_idx = page18_hist_index[curr_slave];
    unsigned short hist_pos = hist_start + hist_idx;
    DataRecord* dev_data = NULL; // 修正：用NULL初始化指针
    rtc_time_t curr_time;

    // 第二步：可执行代码
    LCD_Clear();
    GetCurrentRTC(); // 获取实时RTC时间
    ...
    LCD_DISPLAYCHAR_NEW(0, 16, 0, 15); // ":"
    if (hist_pos < TOTAL_RECORDS && data_summary[hist_pos].is_valid) {
        dev_data = &data_summary[hist_pos];
        // 修正：显式转换为unsigned long
        LCD_DisplayNumber(0, 24, (unsigned long)dev_data->temp, 2); // 温度值（如40）
    } else {
        LCD_DisplayNumber(0, 24, 0UL, 2); // 无数据时显示00（用0UL明确unsigned long类型）
    }
    LCD_DISPLAYCHAR_NEW(0, 40, 0, 2);  // "摄氏度"
//    LCD_DisplayChar(0, 48, 'C');       // "C"
    ...
    if (dev_data != NULL) {
        // 修正：显式转换为unsigned long
        LCD_DisplayNumber(0, 80, (unsigned long)(dev_data->volt1 * 100), 4); // 电压值（如2000）
    } else {
        LCD_DisplayNumber(0, 80, 0UL, 4); // 无数据时显示0000
    }
    // 修正：显式转换为unsigned char*
    LCD_DisplayString(0, 112, (unsigned char*)"mV");   // "mV"

    // 2. 第二行：日期（格式：MM.DD）
    LCD_DISPLAYCHAR_NEW(2, 0, 2, 12);  // "日"
    LCD_DISPLAYCHAR_NEW(2, 8, 3, 12);  // "期"
    LCD_DISPLAYCHAR_NEW(2, 16, 0, 15); // ":"
    LCD_DisplayNumber(2, 24, (unsigned long)curr_time.mon, 2);  // 月（如08）
    LCD_DISPLAYCHAR_NEW(2, 40, 1, 15); // "."                  // "."                  // "."
    LCD_DisplayNumber(2, 48, (unsigned long)curr_time.day, 2);   // 日（如04）

    // 3. 第三行：时间（格式：HH.MM）
    LCD_DISPLAYCHAR_NEW(4, 0, 2, 20);  // "时"
    LCD_DISPLAYCHAR_NEW(4, 8, 3, 20);  // "间"
    LCD_DISPLAYCHAR_NEW(4, 16, 0, 15); // ":"
    LCD_DisplayNumber(4, 24, (unsigned long)curr_time.hour, 2); // 时（如16）
    LCD_DISPLAYCHAR_NEW(4, 40, 0, 15); // ":"
    LCD_DisplayNumber(4, 48, (unsigned long)curr_time.min, 2);  // 分（如20）
...
static void EEPROM_ReadMaxTempRecords(EEPROM_MaxTempRecord records[]) {
    unsigned char i, j;
    unsigned long addr = EEPROM_ADDR_BASE;  // 修正：用unsigned long存储24位地址
    
    // 初始化记录为无效
    for (i = 0; i < EEPROM_MAX_DAYS; i++) {
        memset(&records[i], 0, sizeof(EEPROM_MaxTempRecord));
        records[i].is_valid = 0;
    }
    
    // 开启IAP+24位地址扩展（原函数漏了IAP_CONTR=0x80）
    IAP_CONTR = 0x80;  // 新增：开启IAP功能
    IAP_TPS = 0x02;    // 24MHz时钟对应等待周期
    IAP_CMD |= 0x80;   // 使能24位地址
    
    for (i = 0; i < EEPROM_MAX_DAYS; i++) {
        // 配置24位地址（原函数addr是unsigned int，移位会溢出）
        IAP_ADDRH = (addr >> 16) & 0xFF;
        IAP_ADDRL = (addr >> 8) & 0xFF;
        
        // 逐个字节读取16字节完整记录（原函数只读了2字节）
        for (j = 0; j < sizeof(EEPROM_MaxTempRecord); j++) {
            IAP_CMD = 0x01;  // 读命令
            IAP_TRIG = 0x5A;
            IAP_TRIG = 0xA5;
            _nop_();
            
            // 按结构体顺序存储数据（确保每个字段都读到）
            switch(j) {
                case 0: records[i].pid = IAP_DATA; break;
                case 1: records[i].aid = IAP_DATA; break;
                case 2: records[i].temp = IAP_DATA; break;
                case 3: records[i].volt_mv = (unsigned int)IAP_DATA << 8; break;  // 电压高8位
                case 4: records[i].volt_mv |= IAP_DATA; break;  // 电压低8位
                case 5: records[i].time.year = IAP_DATA; break;
                case 6: records[i].time.mon = IAP_DATA; break;
                case 7: records[i].time.day = IAP_DATA; break;
                case 8: records[i].time.hour = IAP_DATA; break;
                case 9: records[i].time.min = IAP_DATA; break;
                case 10: records[i].time.sec = IAP_DATA; break;
                case 11: records[i].is_valid = IAP_DATA; break;
                default: break;  // 预留字节忽略
            }
...
// 保存当天最高温到EEPROM（第5天覆盖第1天）
static void EEPROM_SaveTodayMaxTemp(EEPROM_MaxTempRecord *new_rec) {
    unsigned long addr = EEPROM_ADDR_BASE;
    unsigned char i, j;
    EEPROM_MaxTempRecord old_records[EEPROM_MAX_DAYS]; // 需提前读取旧数据
     // 新增：先读取EEPROM中已有的4天记录
    EEPROM_ReadMaxTempRecords(old_records);
    
    // 原有循环覆盖逻辑（第5天覆盖第1天）
    for (i = 0; i < EEPROM_MAX_DAYS - 1; i++) {
        old_records[i] = old_records[i + 1];
    }
    old_records[EEPROM_MAX_DAYS - 1] = *new_rec;  // 最新记录存最后一位
    // 1. IAP初始化（规格书要求）
    IAP_CONTR = 0x80;          // 开启IAP
    IAP_TPS = 0x02;           // 24MHz时钟对应等待周期
    IAP_CMD |= 0x80;           // 使能24位地址
    ...
    for (i = 0; i < EEPROM_MAX_DAYS; i++) {
        ...
        // 3.4 写入volt_mv（2字节）
        IAP_CMD = 0x02;
        IAP_DATA = (old_records[i].volt_mv >> 8) & 0xFF;
        IAP_TRIG = 0x5A;
        IAP_TRIG = 0xA5;
        _nop_();
        addr++;
        ...
        IAP_CMD = 0x02;
        IAP_DATA = old_records[i].volt_mv & 0xFF;
        IAP_TRIG = 0x5A;
        IAP_TRIG = 0xA5;
        _nop_();
        addr++;
        ...
        // 3.5 写入time（6字节）
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.year; IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.mon;  IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.day;  IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.hour; IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.min;  IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        IAP_CMD = 0x02; IAP_DATA = old_records[i].time.sec;  IAP_TRIG = 0x5A; IAP_TRIG = 0xA5; _nop_(); addr++; IAP_CMD = 0x00;
        
        // 3.6 写入is_valid（1字节）
        IAP_CMD = 0x02;
        IAP_DATA = old_records[i].is_valid;
        IAP_TRIG = 0x5A;
        IAP_TRIG = 0xA5;
        _nop_();
        addr++;
...
// 删除指定日期的最高温记录（适配PAGE_23功能）

static void EEPROM_DeleteMaxTempRecord(unsigned char day_idx) {
    EEPROM_MaxTempRecord records[EEPROM_MAX_DAYS] = {0};
    EEPROM_ReadMaxTempRecords(records);
    
    // 标记记录为无效
    records[day_idx].is_valid = 0;
    records[day_idx].temp = 0;
    
    // 重新写入EEPROM
    // （复用EEPROM_SaveTodayMaxTemp的写入逻辑，传入修改后的records）
}

// 4. PAGE_21：4天最高温滚动显示（保留原有按键逻辑，仅更新数据来源）
// PAGE_21：4天最高温滚动显示（最终修正版）
static void DisplayPage21(void) {
    const unsigned char row_page[3] = {0, 2, 4}; 
    EEPROM_MaxTempRecord eeprom_records[EEPROM_MAX_DAYS]; // 直接使用结构体类型（无struct）
    unsigned char i;
    unsigned char display_idx;
    unsigned char start_idx = 0;
    unsigned char curr_row;
    
    // 初始化数组（sizeof正常识别结构体大小）
    memset(eeprom_records, 0, sizeof(eeprom_records));
    
    // 读取EEPROM数据（函数参数类型匹配，无错误）
    EEPROM_ReadMaxTempRecords(eeprom_records);
    ...
    for (i = 0; i < 3; i++) {
        ...
        // 选中箭头（条件判断格式规范）
        if ((i) == (menu_state.page21_selected % 3)) {
            LCD_DISPLAYCHAR_NEW(curr_row, 0, 0, 25);
        } else {
            LCD_DisplayChar(curr_row, 0, ' ');
        }
        
        // 数据显示（直接用数组+点访问，无指针，避免C80错误）
        if (eeprom_records[display_idx].is_valid == 1) {
            // 温度
            LCD_DisplayNumber(curr_row, 8, (unsigned long)eeprom_records[display_idx].temp, 2);
            LCD_DISPLAYCHAR_NEW(curr_row, 24, 0, 2); // ℃
            // 日期（月-日）
            LCD_DisplayNumber(curr_row, 40, (unsigned long)eeprom_records[display_idx].time.mon, 2);
            LCD_DISPLAYCHAR_NEW(curr_row, 56, 1, 15); // -
            LCD_DisplayNumber(curr_row, 64, (unsigned long)eeprom_records[display_idx].time.day, 2);
            // 时间（时:分）
            LCD_DisplayNumber(curr_row, 88, (unsigned long)eeprom_records[display_idx].time.hour, 2);
            LCD_DISPLAYCHAR_NEW(curr_row, 104, 0, 15); // :
            LCD_DisplayNumber(curr_row, 112, (unsigned long)eeprom_records[display_idx].time.min, 2);
        } else {
            // 无效数据占位符
            LCD_DisplayString(curr_row, 8, (unsigned char*)"--");
            LCD_DISPLAYCHAR_NEW(curr_row, 24, 0, 2);
            LCD_DisplayString(curr_row, 40, (unsigned char*)"--");
            LCD_DISPLAYCHAR_NEW(curr_row, 56, 1, 15);
            LCD_DisplayString(curr_row, 64, (unsigned char*)"--");
            LCD_DisplayString(curr_row, 88, (unsigned char*)"--");
            LCD_DISPLAYCHAR_NEW(curr_row, 104, 0, 15);
            LCD_DisplayString(curr_row, 112, (unsigned char*)"--");
        }
    }
...
// 5. PAGE_22：显示对应记录详情（完全保留原有固定字符/布局，仅更新数据）
static void DisplayPage22(void) {
    EEPROM_MaxTempRecord eeprom_records[EEPROM_MAX_DAYS] = {0};
    unsigned char selected_idx = menu_state.page21_selected;
    EEPROM_MaxTempRecord *rec = NULL;
    
    // 读取EEPROM数据
    EEPROM_ReadMaxTempRecords(eeprom_records);
    
    // 确定选中的记录（支持4天滚动）
    if (selected_idx < EEPROM_MAX_DAYS) {
        rec = &eeprom_records[selected_idx];
    } else {
        rec = &eeprom_records[selected_idx % EEPROM_MAX_DAYS];
    }
    ...
    LCD_DISPLAYCHAR_NEW(2, 16, 0, 15);
    LCD_DISPLAYCHAR_NEW(2, 64, 2, 20);
    LCD_DISPLAYCHAR_NEW(2, 72, 3, 20);
    LCD_DISPLAYCHAR_NEW(2, 80, 0, 15);
    
    LCD_DISPLAYCHAR_NEW(4, 0, 0, 3);
    LCD_DISPLAYCHAR_NEW(4, 8, 1, 3);
    LCD_DISPLAYCHAR_NEW(4, 16, 0, 15);
    LCD_DisplayString(4, 64, "mV");
    ...
    // 显示选中记录的数据
    if (rec && rec->is_valid) {
        LCD_DisplayNumber(0, 56, rec->pid, 2);
        LCD_DisplayNumber(0, 104, rec->aid, 2);
        LCD_DisplayNumber(2, 24, rec->temp, 2);
        LCD_DisplayChar(2, 40, 'C');
        // 日期时间
        LCD_DisplayNumber(2, 88, rec->time.mon, 2);
        LCD_DisplayChar(2, 104, '-');
        LCD_DisplayNumber(2, 112, rec->time.day, 2);
        // 电压
        LCD_DisplayNumber(4, 24, rec->volt_mv, 4);
    } 
}
...
//                ClearAllHistoryData();
        ...
        if (page18_hist_index[menu_state.page18_selected] > 0) {
            page18_hist_index[menu_state.page18_selected]--;
        } else {
            page18_hist_index[menu_state.page18_selected] = RECORDS_PER_SLAVE - 1;
        }
        display_labels_initialized = 0;
        return;
    } 
...
case 2: // 按键2：PAGE_12返回PAGE_11；PAGE_11返回PAGE_10；PAGE_16返回PAGE_15；PAGE_17返回PAGE_15；PAGE_15返回PAGE_13；其他页下一项
	if (current_page == PAGE_1) {
        ...
        page18_hist_index[menu_state.page18_selected] = 
            (page18_hist_index[menu_state.page18_selected] + 1) % RECORDS_PER_SLAVE;
        display_labels_initialized = 0;
        return;
    }
				else if (current_page == PAGE_20) {
...
//                menu_state.page7_selected--; // 上移=选中索引减1
//                start_idx = GetPage7StartIndex(); // 实时计算新起始索引
//                UpdatePage7MenuItems(start_idx); // 立即更新文本
//                RefreshPage7Arrow(); // 立即刷新箭头
//            }
//            break;
		 if (menu_state.page7_selected > 0) {
...
//                menu_state.page7_selected++; // 下移=选中索引加1
//                start_idx = GetPage7StartIndex(); // 实时计算新起始索引
//                UpdatePage7MenuItems(start_idx); // 立即更新文本
//                RefreshPage7Arrow(); // 立即刷新箭头
//            }
...
static void RefreshDisplay(void)
{
    if (menu_state.page_changed) {
        // 关键：PAGE_2、PAGE_7 不重置display_labels_initialized，避免全量重绘
...
static void CheckAndRecordAlarm(void) {
    ...
    EEPROM_MaxTempRecord new_max_rec;  // 最高温记录结构体（用于存储到EEPROM）
        ...
        if (dev_data != NULL && dev_data->is_valid) {
            ...
            if (current_abnormal == 1) {
                ...
                if (last_abnormal_status[i] == 0) {
                    ...
                    // 新增：将当前异常温度作为当天最高温，存储到EEPROM
                    GetCurrentRTC();  // 获取实时时间（填充到记录中）
                    new_max_rec.pid = dev_data->pid;          // 从站PID
                    new_max_rec.aid = i + 1;                // 从站AID（i+1对应实际AID）
                    new_max_rec.temp = dev_data->temp;       // 异常温度（作为最高温）
                    new_max_rec.volt_mv = dev_data->volt1 * 100; // 电压（转换为mV）
                    new_max_rec.time = current_rtc_time;     // 记录发生时间
                    new_max_rec.is_valid = 1;               // 标记记录为有效
                    new_max_rec.reserved[0] = 0x00;         // 预留字段填0（凑16字节）
                    new_max_rec.reserved[1] = 0x00;
                    new_max_rec.reserved[2] = 0x00;
                    new_max_rec.reserved[3] = 0x00;
                    
                    EEPROM_SaveTodayMaxTemp(&new_max_rec);  // 写入EEPROM（断电保存）
                    
                    // 调试信息：确认最高温已存储
                    UART4_SendString("Max temp saved to EEPROM: AID=");
                    UART4_SendNumber(i + 1, 2);
                    UART4_SendString(", Temp=");
                    UART4_SendNumber(dev_data->temp, 2);
                    UART4_SendString("\r\n");
                ...
                for (j = 0; j < alarm_event_count; j++) {
                    ...
                    if (alarm_events[alarm_idx].is_valid && alarm_events[alarm_idx].aid == (i + 1)) {
                        ...
                        RecordRecoveryEvent(
                            dev_data->pid,        // 从站ID
                            i + 1,                // 区域AID
                            alarm_events[alarm_idx].temp, // 报警时的温度
                            dev_data->temp,       // 恢复后的温度（≤25℃）
                            dev_data->volt1 * 100,// 恢复时的电压（mV）
                            alarm_events[alarm_idx].timestamp // 报警时的时间戳
                        );
                        // 调试信息：确认恢复事件被记录
...
static void GetMaxTempRecords_Ext(short max_temps[], rtc_time_t max_temp_times[], 
                                unsigned char *pid, unsigned char *aid, 
                                unsigned short *volt_mv, DataRecord **target_record) {
    unsigned char slave_idx, hist_idx;
    ...
    for (slave_idx = 0; slave_idx < TOTAL_SLAVES; slave_idx++) {
        ...
        for (hist_idx = 0; hist_idx < RECORDS_PER_SLAVE; hist_idx++) {
            ...
            if (hist_rec->is_valid && hist_rec->temp > 0) {
                ...
                for (i = 0; i < 3; i++) {
                    if (current_temp > max_temps[i]) {
                        // 移位保存前N高温度
                        for (j = 2; j > i; j--) {
                            max_temps[j] = max_temps[j-1];
                            max_temp_times[j] = max_temp_times[j-1];
                        }
                        // 保存当前最高温数据
                        max_temps[i] = current_temp;
                        rtc_read(&max_temp_times[i]);
                        // 保存对应的 PID/AID/电压/记录指针
                        *pid = hist_rec->pid;
                        *aid = hist_rec->aid;
                        *volt_mv = hist_rec->volt1 * 100; // 转换为 mV
                        *target_record = hist_rec;
                        break;
                    }
                }
            }
...
static void GetMaxTempRecords(short max_temps[], rtc_time_t max_temp_times[]) {
    unsigned char dummy_pid = 0;    // 占位变量，无实际用途
    unsigned char dummy_aid = 0;    // 占位变量
    unsigned short dummy_volt = 0;  // 占位变量
    DataRecord *dummy_record = NULL;// 占位变量
    // 调用扩展函数，传递占位变量的地址（满足函数参数要求）
    GetMaxTempRecords_Ext(max_temps, max_temp_times, &dummy_pid, &dummy_aid, &dummy_volt, &dummy_record);
}
