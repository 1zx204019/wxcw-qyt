...
//    
//    // 核心修复1：重置临时编辑数据（避免下次编辑复用旧值）
//    edit_temp_time = current_rtc_time;
//    
//    // 核心修复2：标记显示需要重新初始化（确保下次进入编辑时刷新下划线和数据）
//    display_labels_initialized = 0;
//    
//    rtc_edit_state = RTC_EDIT_IDLE;   // 回到空闲状态
//    UART4_SendString("RTC time saved successfully!\r\n");
...
static void DisplayPage24(void)
{
    ...
    // 第四行: 功能提示（根据状态显示不同的提示）
    if (rtc_edit_state == RTC_EDIT_IDLE) {
        // 浏览模式
        LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);  // "进"
        LCD_DISPLAYCHAR_NEW(6, 8, 1, 4);  // "入"
        LCD_DISPLAYCHAR_NEW(6, 16, 2, 4); // "编"
        LCD_DISPLAYCHAR_NEW(6, 24, 3, 4); // "辑"
    } else if (rtc_edit_state == RTC_EDIT_SELECT) {
        // 选择模式
        LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);  // "上"
        LCD_DISPLAYCHAR_NEW(6, 8, 1, 4);  // "下"
        LCD_DISPLAYCHAR_NEW(6, 16, 5, 4); // "选"
        LCD_DISPLAYCHAR_NEW(6, 24, 6, 4); // "择"
    } else if (rtc_edit_state == RTC_EDIT_CHANGE) {
        // 编辑模式
        LCD_DISPLAYCHAR_NEW(6, 0, 0, 4);  // "加"
        LCD_DISPLAYCHAR_NEW(6, 8, 1, 4);  // "减"
        LCD_DISPLAYCHAR_NEW(6, 16, 5, 4); // "调"
        LCD_DISPLAYCHAR_NEW(6, 24, 6, 4); // "整"
    }
